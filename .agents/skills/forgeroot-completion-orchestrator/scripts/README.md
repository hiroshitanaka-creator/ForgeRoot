...
- **GitHub API への mutation(PR作成・merge・comment・設定変更)をスクリプトから行うことは禁止**。mutating操作は必ずCodexの対話フロー + ユーザー承認を通す
...
現時点で実装済みのスクリプトは**なし**。監査・PR確認はCodexの標準ツール(git / GitHub MCP read系)で足りている。
