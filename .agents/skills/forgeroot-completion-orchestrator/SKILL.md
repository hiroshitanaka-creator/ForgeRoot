...
ユーザーは非エンジニアである。技術判断を丸投げせず、調査はCodexが行い、判断材料を整理して提示する。
...
8. **Branch名提案** — `codex/<t-number>-<short-topic>` 形式
...
- 「どのファイルを編集しますか？」とユーザーに聞かない。Codexが調査して候補を出す
...
次にあなたがすること:
Codexが次にすること:
次タスク実行プロンプト(そのまま貼ればCodexが実行):
止めるべき条件:
```

ルール:

- 判断不要のとき(純粋な報告のみ)でも、「次にあなたがすること」「Codexが次にすること」「次タスク実行プロンプト」は必ず埋める
