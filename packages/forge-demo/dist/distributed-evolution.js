import { ARCHIVE_PACK_SCHEMA_REF, composeCrossRepoPr, enforceNetworkBoundary, exportLineagePack, validateCrossRepoPr, validateLineagePack, validateNetworkBoundary, } from "../../network/dist/index.js";
import { compareArenaCandidates, evaluatePeerReputation, validateArenaComparison, validatePeerReputation, } from "../../eval/dist/index.js";
import { renderFederationReport, validateFederationReport, } from "../../reporting/dist/index.js";
export const DISTRIBUTED_EVOLUTION_DEMO_VERSION = 1;
export const DISTRIBUTED_EVOLUTION_DEMO_SCHEMA_REF = "urn:forgeroot:distributed-evolution-demo:v1";
...
const TOPOLOGY_PATH = "labs/forge-net/topology.yml";
...
export function runDistributedEvolutionDemo(input = {}) {
    ...
    if (!selfValidation.ok)
        return invalidResult(createdAt, topology, selfValidation.issues);
    return result;
}
export function validateDistributedEvolutionDemo(value) {
    const issues = [];
    if (!isRecord(value))
    ...
    if (value.schema_ref !== DISTRIBUTED_EVOLUTION_DEMO_SCHEMA_REF)
        issues.push(issue("schema_ref", "invalid_schema_ref", "schema_ref must identify T070 distributed evolution demo"));
    ...
    if (value.status !== "ready" && value.status !== "blocked" && value.status !== "invalid")
        issues.push(issue("status", "invalid_status", "status must be ready, blocked, or invalid"));
    if (value.decision !== "distributed_evolution_demo_ready" && value.decision !== "distributed_evolution_demo_blocked" && value.decision !== "invalid_distributed_evolution_demo_input")
        issues.push(issue("decision", "invalid_decision", "decision must be a T070 decision"));
    ...
    const expectedDigest = canonicalDigest(demoDigestPayload(value));
    if (typeof value.demo_digest !== "string" || value.demo_digest !== expectedDigest)
        issues.push(issue("demo_digest", "digest_mismatch", "demo_digest must cover the demo payload"));
    if (typeof value.demo_id !== "string" || value.demo_id !== stableId("distributed-evolution-demo", [expectedDigest]))
        issues.push(issue("demo_id", "id_mismatch", "demo_id must match demo_digest"));
...
function invalidResult(createdAt, topology, issues) {
...
function lineageInput(now, sourcePeer, targetPeer, treaty) {
    return {
        now,
        treaty: {
...
function reputationInput(now, targetPeer, treaty) {
    return {
        now,
...
function boundaryInput(now, sourcePeer, targetPeer, treaty, lineagePack, reputation, input) {
    return {
        now,
...
function crossRepoInput(now, targetPeer, lineagePack) {
    return {
        now,
...
function arenaInput(now, crossRepoPr, reputation) {
    return {
        now,
...
function reportInput(now, topology, sourcePeer, targetPeer, treaty, lineagePack, reputation, boundary) {
    return {
        now,
...
function sideEffectGuardIssues(lineagePack, boundary, crossRepoPr, arena, report) {
    const issues = [];
    if (lineagePack.dry_run?.network_transport_performed !== false)
        issues.push(issue("lineagePack.dry_run.network_transport_performed", "network_transport_forbidden", "lineage pack must not perform network transport"));
    if (lineagePack.import_candidate?.adoption_performed !== false)
        issues.push(issue("lineagePack.import_candidate.adoption_performed", "adoption_forbidden", "lineage pack must not adopt imported lineage"));
    if (boundary.dry_run?.network_transport_performed !== false)
        issues.push(issue("networkBoundary.dry_run.network_transport_performed", "network_transport_forbidden", "boundary must not perform network transport"));
    if (boundary.dry_run?.lineage_adoption_performed !== false)
        issues.push(issue("networkBoundary.dry_run.lineage_adoption_performed", "adoption_forbidden", "boundary must not adopt imported lineage"));
    if (boundary.dry_run?.open_federation_enabled !== false)
        issues.push(issue("networkBoundary.dry_run.open_federation_enabled", "open_federation_forbidden", "open federation must stay disabled"));
    if (crossRepoPr.dry_run?.github_api_called !== false || crossRepoPr.dry_run?.pull_request_created !== false)
        issues.push(issue("crossRepoPr.dry_run", "github_transport_forbidden", "cross-repo PR composition must not call GitHub or create a PR"));
    if (arena.dry_run?.automatic_merge_performed !== false || arena.dry_run?.network_transport_performed !== false)
        issues.push(issue("arena.dry_run", "arena_side_effect_forbidden", "arena must not merge or transport"));
    if (report.report_context?.source_of_truth !== false || report.report_context?.treaty_replacement !== false)
...
function chainStatusIssues(lineagePack, peerReputation, boundary, crossRepoPr, arena, report) {
    ...
    if (crossRepoPr.status !== "cross_repo_pr_ready")
        issues.push(issue("crossRepoPr.status", "cross_repo_pr_not_ready", "cross-repo PR composition must be ready"));
    if (arena.status !== "arena_ready")
        issues.push(issue("arena.status", "arena_not_ready", "arena must select a ready winner"));
...
function validateChain(chain, summary, status, issues) {
    ...
    for (const spec of CHAIN_VALIDATORS) {
        ...
        if (manifest === undefined) {
            if (requiresCompleteChain)
                issues.push(issue(`chain.${spec.field}`, "chain_manifest_required", `${spec.field} is required for non-invalid results`));
            if (validation?.[spec.summaryKey] === true)
                issues.push(issue(`validation_summary.${spec.summaryKey}`, "validation_summary_mismatch", `${spec.summaryKey} cannot be true when the chain manifest is missing`));
            continue;
        }
...
function validateReadBackConsistency(summary, invariants, status, issues) {
    const validation = asRecord(summary);
    const invariantRecord = asRecord(invariants);
    if (validation === null || invariantRecord === null)
        return;
    if (!VALIDATION_SUMMARY_KEYS.every((key) => typeof validation[key] === "boolean"))
        return;
...
function validateStatusConsistency(value, issues) {
    if (value.status === "ready" && value.decision !== "distributed_evolution_demo_ready")
        issues.push(issue("decision", "status_decision_mismatch", "ready status requires distributed_evolution_demo_ready"));
    if (value.status === "blocked" && value.decision !== "distributed_evolution_demo_blocked")
        issues.push(issue("decision", "status_decision_mismatch", "blocked status requires distributed_evolution_demo_blocked"));
    if (value.status === "invalid" && value.decision !== "invalid_distributed_evolution_demo_input")
        issues.push(issue("decision", "status_decision_mismatch", "invalid status requires invalid_distributed_evolution_demo_input"));
...
function validateTopologyRef(value, issues) {
    const ref = asRecord(value);
    if (ref === null) {
        issues.push(issue("topology_ref", "topology_ref_required", "topology_ref is required"));
        return;
    }
    if (typeof ref.topology_id !== "string" || ref.topology_id.length === 0)
        issues.push(issue("topology_ref.topology_id", "invalid_topology_id", "topology_id is required"));
    if (ref.topology_path !== TOPOLOGY_PATH)
        issues.push(issue("topology_ref.topology_path", "topology_path_mismatch", "topology path must point to the T069 lab topology"));
    if (ref.scope !== "lab_only")
        issues.push(issue("topology_ref.scope", "lab_scope_required", "scope must be lab_only"));
    if (ref.source_of_truth !== false)
        issues.push(issue("topology_ref.source_of_truth", "source_of_truth_forbidden", "demo topology must not replace source of truth"));
}
function validateSteps(value, issues) {
    ...
    for (const [index, item] of value.entries()) {
        ...
        if (name === null)
            issues.push(issue(`steps.${index}.name`, "name_required", "step name is required"));
        for (const key of ["status", "produced"])
            if (typeof step[key] !== "string" || step[key].length === 0)
                issues.push(issue(`steps.${index}.${key}`, "string_required", `${key} is required`));
        if (!(step.id === null || typeof step.id === "string"))
            issues.push(issue(`steps.${index}.id`, "invalid_id", "step id must be null or string"));
    }
}
function validateSummary(value, issues) {
    ...
    for (const key of ["route_id", "source_peer_id", "target_peer_id"]) {
        if (!(typeof summary[key] === "string" && summary[key].length > 0) && summary[key] !== null)
            issues.push(issue(`summary.${key}`, "invalid_summary_ref", `${key} must be string or null`));
    }
    for (const key of ["lineage_pack_id", "boundary_decision_id", "cross_repo_composition_id", "arena_result_id", "federation_report_id", "arena_winner_candidate_id"]) {
        if (!(summary[key] === null || typeof summary[key] === "string"))
            issues.push(issue(`summary.${key}`, "invalid_summary_id", `${key} must be string or null`));
    }
}
function validateInvariants(value, issues) {
    ...
    if (typeof invariants.all_read_back_validations_passed !== "boolean")
        issues.push(issue("invariants.all_read_back_validations_passed", "boolean_required", "all_read_back_validations_passed must be boolean"));
}
function validateValidationSummary(value, issues) {
    const summary = asRecord(value);
    if (summary === null) {
    ...
    for (const key of VALIDATION_SUMMARY_KEYS) {
        if (typeof summary[key] !== "boolean")
            issues.push(issue(`validation_summary.${key}`, "boolean_required", `${key} must be boolean`));
    }
}
function validateTopology(topology) {
    const issues = [];
    if (topology.schema_ref !== "urn:forgeroot:forge-net-topology:v1")
        issues.push(issue("topology.schema_ref", "invalid_topology_schema", "topology must be T069 forge-net topology v1"));
    if (topology.manifest_version !== 1)
        issues.push(issue("topology.manifest_version", "invalid_topology_version", "topology manifest_version must be 1"));
    if (topology.scope !== "lab_only")
...
function normalizeTopology(value) {
    ...
    const repositories = arrayValue(value.repositories).map(asRecord).filter((entry) => entry !== null).map((entry) => ({
    ...
    const treatyLinks = arrayValue(value.treaty_links).map(asRecord).filter((entry) => entry !== null).map((entry) => ({
    ...
    const lineageRoutes = arrayValue(value.lineage_routes).map(asRecord).filter((entry) => entry !== null).map((entry) => ({
...
function candidate(candidate_id, source_kind, proposal_ref, eval_score, lineage_score, reputation_score, risk, evidence_digest) {
    return {
        candidate_id,
        source_kind,
...
function step(name, status, produced, id) {
    return { name, status, produced, id };
}
function demoDigestPayload(value) {
    return {
...
function validateNoSecretMaterial(value, path, issues) {
    if (typeof value === "string") {
        ...
        if (lower.includes("bearer ") || lower.includes("ghp_") || lower.includes("github_pat_") || lower.includes("-----begin") || lower.includes("private_key"))
            issues.push(issue(path, "secret_material_forbidden", "demo manifests must not contain token or private-key material"));
        return;
    }
    if (Array.isArray(value)) {
...
//# sourceMappingURL=distributed-evolution.js.map