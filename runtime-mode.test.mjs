...
export async function routeRequest(
  ...
  if (pathname === "/api/forge/kill-switch") {
    await routeKillSwitch(request, response, options);
    return;
  }
...
async function routeKillSwitch(
  request: IncomingMessage,
  response: ServerResponse,
  options: GitHubWebhookServerOptions,
): Promise<void> {
  if (options.runtimeController === undefined || options.adminToken === undefined || options.adminToken.length === 0) {
    sendJson(response, 404, { ok: false, error: "not_found" });
    return;
  }
  ...
  const actor = readPayloadString(payload, "actor") ?? "github-app://forgeroot";
  const reason = readPayloadString(payload, "reason") ?? "manual kill switch";
  const correlationId = readPayloadString(payload, "correlation_id");
  const snapshot = options.runtimeController.activateKillSwitch({ actor, reason, correlationId });
...
export function respondToWebhookDecision(
  response: ServerResponse,
