...
export type EventInboxEnqueueResult =
  | { kind: "inserted"; record: EventInboxRecord }
  | { kind: "duplicate"; record: EventInboxRecord }
  | {
      kind: "conflict";
...
export function openSqliteEventInbox(databasePath = ":memory:"): SqliteEventInbox {
...
export class SqliteEventInbox implements EventInbox {
  ...
  enqueue(delivery: AcceptedWebhookDelivery, now = new Date()): EventInboxEnqueueResult {
  ...
  markProcessed(deliveryId: string, now = new Date()): EventInboxRecord | null {
    const nowIso = now.toISOString();
    const result = this.db
      ...
      .run(nowIso, deliveryId) as StatementResult;
  ...
  markFailed(deliveryId: string, options: MarkFailedOptions): EventInboxRecord | null {
    const now = options.now ?? new Date();
    const nowIso = now.toISOString();
    const status: EventInboxStatus = options.retryable ? "failed_retryable" : "failed_terminal";
    ...
    const result = this.db
      ...
      .run(status, nextAttemptAt, errorMessage, nowIso, deliveryId) as StatementResult;
  ...
  private getOrThrow(deliveryId: string): EventInboxRecord {
...
export function isEventInboxEnqueueResult(value: unknown): value is EventInboxEnqueueResult {
...
function rowToRecord(row: SqliteRow): EventInboxRecord {
...
function readNumber(row: SqliteRow, key: string): number {
  const value = row[key];
...
function readNullableNumber(row: SqliteRow, key: string): number | null {
  const value = row[key];
  if (value === null || value === undefined) {
    return null;
  }
...
function readHash(row: SqliteRow, key: string): `sha256:${string}` {
...
function readStatus(row: SqliteRow, key: string): EventInboxStatus {
  const value = readString(row, key);
  switch (value) {
    case "received":
    case "processing":
